...
data_path = 'data/data_657.csv'
dataset = pd.read_csv(data_path)[['G', 'N', 'E', 'O', 'A', 'C']]
# 行数
rows = dataset.shape[0]
m_dataset = dataset.drop(index=dataset.loc[(dataset['G'] == 0)].index)[['N', 'E', 'O', 'A', 'C']]
f_dataset = dataset.drop(index=dataset.loc[(dataset['G'] == 1)].index)[['N', 'E', 'O', 'A', 'C']]
# calculate std mean
...
for i in system3.columns:
    ...
    print(i, ':', amount)
system3.insert(0, 'No', value=list(pd.read_csv(data_path)['No']))
system3.to_csv('data/label_657.csv', sep=',', encoding='utf-8', index=False)
