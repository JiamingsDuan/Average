# -*- coding: utf-8 -*-
"""
Created on Thu Jun 13 19:54:59 2019
...
y=label
 #划分测试集，训练集
x_train0, x_test0, y_train0, y_test0 =  model_selection.train_test_split( x, y, test_size=0.4,random_state=2)
def make_pca(i):
    u=np.mean(image, axis=1)    #求平均值    #注：求的是平均脸，既某位置400人脸的平均值，而不是人的平均值
    A=[]
    for t in range(1024):
...
#        i=i+1
    ...
    for i in lis:
        model = sk_svm.SVC(C=1/n, kernel=i)
        model.fit(P_pca,y_train0)
        acc=model.score(Q_pca,y_test0) #根据给定数据与标签返回正确率的均值
        acc_aa.append(acc)
        print('SVM模型',i,'评价:',acc)
